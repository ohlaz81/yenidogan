...
async function main() {
  ...
  if (seeds.length === 0) {
    throw new Error("fgh-girl-* seed kaydı bulunamadı.");
  }
  ...
  const verifySlugs = (rowsToInsert.length > 0 ? rowsToInsert : seeds.map((seed) => toDbRow(seed, now)))
    .slice(0, 5)
    .map((row) => row.slug);
  ...
  if (verifyError) {
    throw new Error(`Doğrulama sorgusu başarısız: ${verifyError.message}`);
  }

  const listChecks: Record<string, number> = {};
  for (const letter of ["F", "G", "H"]) {
    const { data, error } = await supabase
      ...
      .in(
        "slug",
        seeds.filter((seed) => seed.displayName.slice(0, 1).toLocaleUpperCase("tr-TR") === letter).map((seed) => seed.slug),
      );
