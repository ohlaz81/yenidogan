...
function isKlmGirlSeed(seed: (typeof BABY_NAME_SEED)[number]) {
  const first = seed.displayName.slice(0, 1).toLocaleUpperCase("tr-TR");
  return seed.id.startsWith("klm-girl-") && seed.gender === "GIRL" && ["K", "L", "M"].includes(first);
}
...
async function fetchRelationsForSources(sourceIds: string[]) {
  const out: Array<{ sourceId: string; targetId: string }> = [];
  const pageSize = 1000;
  for (let from = 0; ; from += pageSize) {
    ...
    if (error) {
      throw new Error(`SimilarName kontrolü başarısız: ${error.message}`);
    }
    out.push(...((data ?? []) as Array<{ sourceId: string; targetId: string }>));
...
async function main() {
  const seeds = BABY_NAME_SEED.filter(isKlmGirlSeed);
  ...
  const existingRelations = await fetchRelationsForSources(relationSourceIds);
  ...
  const verifyRelations = await fetchRelationsForSources(sourceIds);
  ...
  const duplicatePairCount =
    verifyRelations.length - new Set(verifyRelations.map((row) => relationKey(String(row.sourceId), String(row.targetId)))).size;
    ...
    JSON.stringify(
      {
        ...
        duplicatePairCount,
        allSourcesAtLeastEight: belowTarget.length === 0,
        belowTarget,
        relationCounts,
        insertedRelationSamples: toInsert.slice(0, 40).map(({ sourceSlug, targetSlug }) => ({ sourceSlug, targetSlug })),
