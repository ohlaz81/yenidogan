...
function isSGirlSeed(seed: (typeof BABY_NAME_SEED)[number]) {
  const first = seed.displayName.slice(0, 1).toLocaleUpperCase("tr-TR");
  return seed.id.startsWith("s-girl-") && seed.gender === "GIRL" && first === "S";
}
...
async function main() {
  const seeds = BABY_NAME_SEED.filter(isSGirlSeed);
