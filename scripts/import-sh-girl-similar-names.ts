...
function isShGirlSeed(seed: (typeof BABY_NAME_SEED)[number]) {
  const first = seed.displayName.slice(0, 1).toLocaleUpperCase("tr-TR");
  return seed.id.startsWith("sh-girl-") && seed.gender === "GIRL" && first === "Ş";
}
...
async function main() {
  const seeds = BABY_NAME_SEED.filter(isShGirlSeed);
