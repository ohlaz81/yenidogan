...
function isTGirlSeed(seed: (typeof BABY_NAME_SEED)[number]) {
  const first = seed.displayName.slice(0, 1).toLocaleUpperCase("tr-TR");
  return seed.id.startsWith("t-girl-") && seed.gender === "GIRL" && first === "T";
}
...
async function main() {
  const seeds = BABY_NAME_SEED.filter(isTGirlSeed);
