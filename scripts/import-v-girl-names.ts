...
async function main() {
  ...
  if (seeds.length === 0) {
    throw new Error("v-girl-* seed kaydı bulunamadı.");
  }
  ...
  if (listError) {
    throw new Error(`V harfi liste kontrolü başarısız: ${listError.message}`);
  }
    ...
    JSON.stringify(
      {
        ...
        listCheck: listData?.length ?? 0,
        cacheNote: "Name.updatedAt sürümü değiştiği için yayınlanmış isim liste cache'i otomatik yeni anahtara geçer.",
      },
      null,
      2,
    ),
