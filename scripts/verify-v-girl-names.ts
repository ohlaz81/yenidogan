...
function isVGirlSeed(seed: (typeof BABY_NAME_SEED)[number]) {
  const first = seed.displayName.slice(0, 1).toLocaleUpperCase("tr-TR");
  return seed.id.startsWith("v-girl-") && seed.gender === "GIRL" && first === "V";
}
...
async function main() {
  const seeds = BABY_NAME_SEED.filter(isVGirlSeed);
  const seedSlugs = seeds.map((seed) => seed.slug);
  const sampleSlugs = ["vera", "vesile", "veda", "vildan", "vuslat"];
  const pageCheckSlugs = ["vera", "vesile", "veda", "vela"];

  const { data: rows, error } = await supabase
    .from("Name")
    .select("id,slug,displayName,gender,meaning,style,isShort,beautifulMeaning,firstLetter,popularScore,published")
    .in("slug", [...seedSlugs, "vela"]);

  if (error) throw new Error(`V isimleri alınamadı: ${error.message}`);

  const names = (rows ?? []) as NameLite[];
  const seedNames = names.filter((name) => seedSlugs.includes(name.slug) && name.gender === "GIRL");
  const ids = seedNames.map((name) => name.id);
  const { data: similarRows, error: similarError } = await supabase
    .from("SimilarName")
    .select(
      "sourceId,target:Name!SimilarName_targetId_fkey(id,slug,displayName,gender,meaning,style,isShort,beautifulMeaning,firstLetter,popularScore,published)",
    )
    .in("sourceId", ids);
  ...
  for (const row of (similarRows ?? []) as unknown as SimilarRow[]) {
    if (!row.target || row.target.gender !== "GIRL") continue;
  ...
  const relationCounts = Object.fromEntries(seedNames.map((name) => [name.slug, similarBySource.get(name.id)?.length ?? 0]));
  const missingRelationCount = Object.values(relationCounts).filter((count) => count < 8).length;
  const duplicateCount =
    (similarRows?.length ?? 0) -
    new Set((similarRows ?? []).map((row) => `${String(row.sourceId)}->${String((row as unknown as SimilarRow).target?.id ?? "")}`))
      .size;
    ...
    JSON.stringify(
      {
        ...
        dbNameCount: seedNames.length,
        relationCounts,
        duplicateCount,
        missingRelationCount,
        pageChecks: {
          "/harf/v": seedNames.some((name) => name.firstLetter === "V"),
          checkedSlugs: Object.fromEntries(pageCheckSlugs.map((slug) => [`/isim/${slug}`, bySlug.has(slug)])),
          velaIncludedAsGirl: bySlug.get("vela")?.gender === "GIRL",
        },
        sample,
      },
      null,
