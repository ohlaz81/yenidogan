...
export function NameCard({ name, index }: { name: NameWithImage; index?: number }) {
  return (
    <article className="relative flex flex-col gap-3 rounded-2xl border border-border bg-card p-4 shadow-sm sm:flex-row">
      {typeof index === "number" && (
        <div className={nameIndexBadgeClass(name.gender)}>{index + 1}</div>
      )}
      <div className={`order-2 min-w-0 flex-1 space-y-2 sm:order-1 ${typeof index === "number" ? "pt-6" : ""}`}>
        <div className="flex items-start justify-between gap-2">
          <div>
            <Link
              href={`/isim/${name.slug}`}
              className={`font-display text-xl font-semibold hover:underline ${nameDisplayTextClass(name.gender)}`}
            >
              {name.displayName}
            </Link>
            <div className="mt-1 flex flex-wrap gap-1">
              <span className={nameGenderBadgeClass(name.gender)}>{genderLabels[name.gender]}</span>
